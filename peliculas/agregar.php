<?php
// Primero, incluye tus archivos de configuración y base de datos.
require_once '../includes/config.php';
require_once '../includes/db.php';

// Asegúrate de que la sesión esté iniciada para 'auth.php' y para obtener el usuario_id
if (session_status() !== PHP_SESSION_ACTIVE) {
    session_start();
}

// Luego, la lógica de autenticación que podría redirigir
...
// VERIFICACIÓN IMPORTANTE: Asegúrate de que $_SESSION['usuario_id'] esté definido.
// Si auth.php no maneja esto, podrías necesitar una redirección aquí.
if (!isset($_SESSION['usuario_id'])) {
    header('Location: ' . APP_URL . 'login.php'); // Redirige si el usuario no está logueado
    exit;
}
$usuario_id = $_SESSION['usuario_id'];


// **** COMIENZO DEL BLOQUE DE LÓGICA DE PROCESAMIENTO ****
// Este bloque debe ejecutarse ANTES de cualquier salida HTML.
    ...
    if (!$titulo || !$genero || !$plataforma) {
        ...
        if (!$mensaje) {
            ...
            if ($resultado) {
                ...
                // Esta es la línea que antes fallaba. Ahora se ejecuta antes de cualquier HTML.
                header('Location: ' . APP_URL . 'dashboard.php');
                exit; // SIEMPRE usa exit; después de header('Location: ...')
            } else {
...
// **** FIN DEL BLOQUE DE LÓGICA DE PROCESAMIENTO ****

// **** COMIENZO DEL BLOQUE HTML ****
// Ahora sí, incluye el header (que a su vez incluirá la navbar)
// y luego el resto del HTML.
